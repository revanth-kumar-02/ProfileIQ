...
/**
 * DirectLinkedInProvider
 *
 * Diagnostic/Development provider performing direct HTTP fetch requests to public LinkedIn profile URLs.
 * Used for development diagnostics and detecting public profile responses.
 *
 * NOTE: Direct HTTP requests to LinkedIn URLs are blocked by LinkedIn anti-bot edge controls (HTTP 999).
 * This provider is for diagnostic purposes only and will return structured PROFILE_ACCESS_BLOCKED error results when blocked.
 */
export class DirectLinkedInProvider implements ProfileExtractionProvider {
  id = 'direct-linkedin';
  name = 'Direct LinkedIn Fetch Provider (Development)';

  async isAvailable(): Promise<boolean> {
    // Direct LinkedIn provider is always available for local dev/diagnostics
    return true;
  }
  ...
  async extractProfile(profileUrl: string): Promise<ExtractionResult> {
    ...
    console.log(`[Extraction Provider] Provider started: ${this.name} (${this.id})`);
...
export const LinkedInExtractionProvider = DirectLinkedInProvider;
